...
@Component({
  selector: 'app-agregar-cocinero-dialog',
  templateUrl: './agregar-cocinero-dialog.component.html',
  styleUrls: ['./agregar-cocinero-dialog.component.css']
})
export class AgregarCocineroDialogComponent implements OnInit {
  ...
  agregarCocinero() {
    ...
    if(this.validarString([nombre, password, usuario, estado])) {
      this._AdminServiceApi.agregarCocinero(this.form.value).subscribe({
        next: (x) => {
          if(x.status == "ok"){
            alert(`Se agrego el cocinero/a: ${nombre.trim()}`)
